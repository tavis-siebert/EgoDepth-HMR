...
from .utils_depth_data import get_example, get_example_hha
...
class ImageDatasetHHAEgoBody(Dataset):
    ...
    def __init__(self,
        ...
        """
        super(ImageDatasetHHAEgoBody, self).__init__()
        self.train = train
        self.split = split
        self.cfg = cfg
        self.device = device
        ...
        # check if the each image actually exists
        hha_imgname = []
        for x in self.imgname:
            if exists(x.replace('egocentric_depth_processed', 'egocentric_depth_hha')):
                hha_imgname.append(x.replace('egocentric_depth_processed', 'egocentric_depth_hha'))
        self.imgname = hha_imgname[::spacing]
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # image_file = image_file.replace('egocentric_depth_processed', 'egocentric_depth_hha')
        ...
        hha_img, keypoints_3d_auge, smpl_params = get_example_hha(image_file, keypoints_3d, smpl_params,
        ...
        #                              [0., 200., 144.],
        #                              [0., 0., 1.]])
        # hha_img = getHHA(camera_matrix, img, img)
        # hha_img = np.transpose(hha_img, (2, 0, 1))  # [3, h, w]
        # hha_img = hha_img.astype(np.float32) / 255.0
