...
from ..utils.depth2surfNorm import compute_normals_from_depth_batch, compute_normals_from_depth


class ImageDatasetSurfnormalsEgoBody(Dataset):
    ...
    def __init__(self,
        ...
        """
        super(ImageDatasetSurfnormalsEgoBody, self).__init__()
        self.train = train
        self.split = split
        self.cfg = cfg
        self.device = device
        ...
        self.body_pose = self.data['body_pose'].astype(np.float32)[::spacing]  # [n_sample, 69]
        self.betas = self.data['betas'].astype(np.float32)[::spacing]
        self.global_orient_depth = self.data['global_orient_depth'].astype(np.float32)[::spacing]  # [n_sample, 3]
        self.transl_depth = self.data['transl_depth'].astype(np.float32)[::spacing]
        self.keypoints_3d_depth = self.data['3d_joints_depth'].astype(np.float32)[::spacing]
        ...
        if self.with_masks:
            ...
            if not exists(self.mask_dir):
                print('[WARNING] mask_dir {} does not exist, set with_masks to False.'.format(self.mask_dir))
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        surf_normal = compute_normals_from_depth(img)
        
        item['img'] = img
        item['surf_normals'] = surf_normal
        ...
        if self.with_masks:
            # get mask
            mask_file = join(self.mask_dir, f'mask_{idx}.png')
            if not exists(mask_file):
                print('[WARNING] mask file {} does not exist.'.format(mask_file))
                item['mask'] = None
            else:
                mask = cv2.imread(mask_file, cv2.IMREAD_GRAYSCALE)
                item['mask'] = mask.astype(np.int32)  # normalize to [0, 1]
