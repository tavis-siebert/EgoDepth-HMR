...
from prohmr.datasets.image_dataset_hha_egobody import ImageDatasetHHAEgoBody

from utils import *
from prohmr.models import ProHMRRGBSmplx, ProHMRHHAEgobody
...
def train(writer, logger):
    ...
    if args.data_source != 'mix':
        train_dataset = ImageDatasetHHAEgoBody(cfg=model_cfg, train=True, device=device, img_dir=args.train_dataset_root,
    ...
    val_dataset = ImageDatasetHHAEgoBody(cfg=model_cfg, train=False, device=device, img_dir=args.val_dataset_root,
    ...
    # Setup model
    model = ProHMRHHAEgobody(cfg=model_cfg, device=device, writer=None, logger=None, with_global_3d_loss=args.with_global_3d_loss)
    model.train()
