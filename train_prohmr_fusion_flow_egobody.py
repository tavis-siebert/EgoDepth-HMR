...
def train(writer, logger):
    ...
    if args.load_depth_pretrained:
        ...
        weights_backbone['state_dict'] = {k.replace('flow', 'flow_depth'): v for k, v in weights_backbone['state_dict'].items()}
        model.flow.flow_depth.load_state_dict(weights_flow['state_dict'], strict=False)
    ...
    if args.load_rgb_pretrained:
        ...
        weights_flow = {}
        weights_flow['state_dict'] = {k: v for k, v in weights['state_dict'].items() if k.split('.')[0] == 'flow'}
        weights_backbone['state_dict'] = {k.replace('flow', 'flow_surfnorms'): v for k, v in weights_backbone['state_dict'].items()}
        model.flow.flow_surfnorms.load_state_dict(weights_flow['state_dict'], strict=False)
