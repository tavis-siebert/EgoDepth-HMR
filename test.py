...
from prohmr.utils.geometry import render_keypoints_to_depth_map_fast, perspective_projection, project_on_depth_torch_batch, crop_around_bbox_center
from prohmr.utils.visualize import save_depth_image, save_reprojection_images
from prohmr.utils.renderer import *
...
for step, batch in enumerate(tqdm(dataloader)):
    if 1:
        ...
        if step == 0:
            # focal_length = torch.tensor([5000.,5000.]).repeat((curr_batch_size , 1)).to(device)  # [bs, 2, 1]
            # cam_t = torch.stack([gt_pose['transl'][:, 1], gt_pose['transl'][ :, 2],
            #                         2 * focal_length[:, 0] / (model_cfg.MODEL.IMAGE_SIZE * gt_pose['transl'][ :, 0] + 1e-9)],
            #                         dim=-1)
            # cam_t = cam_t.reshape(-1, 3)
            # focal_length = focal_length.reshape(-1, 2)
            # camera_center = torch.tensor([160., 144.]).repeat((curr_batch_size , 1)).to(device)  # [bs , 2]

            
            # cam_t_full = convert_pare_to_full_img_cam(pare_cam=gt_pose['transl'].reshape(-1, 3), bbox_height=torch.tensor([200]).to(device).unsqueeze(1).repeat(1, 1).reshape(-1),
            #                                                bbox_center=camera_center.unsqueeze(1).repeat(1, 1, 1).reshape(-1, 2),
            #                                                # img_w=1920, img_h=1080,
            #                                                img_w=camera_center[:, 0]*2, img_h=camera_center[:, 1]*2,
            #                                                focal_length=focal_length[:, 0],
            #                                                crop_res=model_cfg.MODEL.IMAGE_SIZE)
            
            # reprojected_vertices, depths_vertices = perspective_projection(gt_vertices.reshape(-1, 10475, 3), 
            #                                             translation=cam_t_full,
            ...
            depth_maps = project_on_depth_torch_batch(gt_vertices.reshape(-1, 1, 10475, 3), intrinsic_matrix, width, height)
